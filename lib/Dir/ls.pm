...
Sort by ctime in seconds since the epoch.
...
Specify sort algorithm. Valid values are: C<none>, C<extension>, C<size>,
C<time>, or C<version>.
...
Sort by mtime in seconds since the epoch. Equivalent to C<< sort => 'time' >>.

=item u

Sort by atime in seconds since the epoch.

=item U

Return entries in filesystem order (unsorted). Equivalent to
C<< sort => 'none' >>.
